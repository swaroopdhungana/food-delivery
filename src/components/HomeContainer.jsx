    ...
    >
      <div className="py-2 flex-1 flex flex-col items-start justify-center  gap-6">
        ...
        <motion.button
          whileTap={{ scale: 0.75 }}
          type="button"
          className="bg-gradient-to-br from-[#fa80af] to-[#f0508e] w-full px-4 py-3 font-bold rounded-lg hover:shadow-xl transition-all ease-in-out duration-150 md:w-auto text-green-100 tracking-wider"
        >
          Order Now
        </motion.button>
      </div>
      <div className="py-2 flex-1 flex items-center relative">
        <img
          ...
          alt="hero-bg"
        />
        <div className="w-full h-full absolute top-0 left-0 flex items-center p-8 justify-center lg:px-32 flex-wrap gap-4">
          {ITEM_DATA.map((item) => {
            return (
              <div
                key={item.id}
                className="lg:w-[200px] bg-cardOverlay backdrop-blur-md rounded-xl flex flex-col items-center justify-center shadow-lg"
              >
                <img
                  src={item.img}
                  alt="products"
